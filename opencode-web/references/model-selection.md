...
+   - Check if model is available in `/config/models`
+   - Use the model ID in the request
...
+   - Use the default model from `/config/models` (if available)
+   - Fall back to the first available model
 
 4. **Model ID format:**
-   - Always use `providerID:modelID` format
-   - Examples: `openai:gpt-4`, `anthropic:claude-3-opus`, `ollama:llama3`
+   - Use the `id` string returned by the server
+   - Examples: `openai:gpt-4`, `anthropic:claude-3-opus`, `ollama:llama3` (usually `provider:name`)
...
+| Model not found | Verify model ID format matches available models from `/config/models` |
+| Server error | Ensure `opencode serve` is running and accessible |
...
+3. **Cache model list** to avoid repeated calls
